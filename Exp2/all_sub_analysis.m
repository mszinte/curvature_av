...
sub.numSjct         =   7;                                      % number of subjects
sub.tasks           =   { 'AVDist'};                            % task names
...
sub.total_trials = [];
sub.repeat_trials = [];
sub.online_correct_trials = [];
sub.offline_correct_trials = [];
for numSjct = 1:sub.numSjct
    ...
    for taskNum = 1:size(sub.tasks,2)
        ...
        for blockNum = 1:sub.blocks
            ...
            behav_mat = csvread(sprintf('%s/%s_task-%s_%s_BehavData.csv',sub.raw_filedir,sub.ini,sub.taskName,sub.blockNum));
            total_trials = total_trials + size(behav_mat,1);
            repeat_trials = repeat_trials + size(behav_mat(behav_mat(:,13)~=1,:),1);
        end
        
    end
    sub.total_trials = [sub.total_trials;total_trials];
    sub.repeat_trials = [sub.repeat_trials;repeat_trials];
    sub.online_correct_trials = [sub.online_correct_trials;total_trials-repeat_trials];
    
    sub.deriv_filedir = sprintf('%s/OSF_deriv_data/%s',sub.dirF,sub.ini);
    offline_correct_mat = csvread(sprintf('%s/%s_Tab.csv',sub.deriv_filedir,sub.ini));
    sub.offline_correct_trials = [sub.offline_correct_trials;size(offline_correct_mat,1)];
    
end
sub.total_total_trials = sum(sub.total_trials);
sub.total_repeat_trials = sum(sub.repeat_trials);
sub.total_online_correct_trials = sum(sub.online_correct_trials);
sub.total_offline_correct_trials = sum(sub.offline_correct_trials);
